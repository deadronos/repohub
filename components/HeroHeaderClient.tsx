...
type HeroHeaderProps = React.ComponentProps<typeof HeroHeader>;
...
export default function HeroHeaderClient(props: HeroHeaderProps) {
  ...
  return (
    <header className="relative grid gap-8 lg:grid-cols-[minmax(0,1.3fr)_minmax(320px,0.7fr)] lg:items-end">
      <div className="space-y-7 text-center lg:text-left">
        ...
        {/* Title + Subtitle */}
        <motion.div custom={1} variants={stagger} initial="hidden" animate="visible" className="mx-auto max-w-4xl lg:mx-0">
        ...
        {/* Buttons */}
        <motion.div custom={2} variants={stagger} initial="hidden" animate="visible" className="flex flex-col items-center gap-3 sm:flex-row sm:justify-center lg:justify-start">
          <a
            href="#projects"
          ...
          >
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="h-5 w-5 transition-transform duration-300 group-hover:rotate-12"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg>
            @deadronos
          </a>
        </motion.div>
      </div>
      ...
      {/* Stats panel */}
      <motion.aside custom={3} variants={stagger} initial="hidden" animate="visible" className="glass-panel rounded-3xl border border-white/10 p-5 shadow-[0_0_40px_rgba(2,8,23,0.45)] md:p-6">
        ...
        <dl className="mt-5 grid grid-cols-2 gap-3">
          <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
            ...
            <dd className="mt-2 text-3xl font-semibold text-white">{props.projectCount}</dd>
          </div>

          <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
            <dt className="text-xs uppercase tracking-[0.26em] text-zinc-500">Featured</dt>
            <dd className="mt-2 text-3xl font-semibold text-white">{props.featuredCount}</dd>
          </div>
          ...
          <div className="col-span-2 rounded-2xl border border-white/10 bg-white/[0.03] p-4">
            ...
            <dd className="mt-2 text-lg font-medium text-zinc-100">{props.latestProjectLabel}</dd>
          </div>
          ...
          <div className="col-span-2 rounded-2xl border border-white/10 bg-white/[0.03] p-4">
            ...
            <dd className="mt-2 inline-flex rounded-full border border-cyan-400/20 bg-cyan-400/10 px-3 py-1 text-sm font-medium text-cyan-100">
              {props.topTag.trim() ? props.topTag.charAt(0).toUpperCase() + props.topTag.slice(1) : 'Open source'}
            </dd>
          </div>
        </dl>
      </motion.aside>
