  ...
  return (
    <div className="min-h-screen p-8 max-w-6xl mx-auto">
      ...
      <div className="flex justify-between items-center mb-8 border-b border-zinc-800 pb-4">
        ...
        <Link href="/" className="text-sm bg-zinc-800 hover:bg-zinc-700 text-zinc-300 px-4 py-2 rounded-lg transition-colors flex items-center gap-2">
            ← Back to Base
        </Link>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {projects.map(project => (
            <div key={project.id} className="relative group bg-zinc-900/50 border border-zinc-800 rounded-xl overflow-hidden hover:border-cyan-500/30 transition-all">
                {/* Selection Checkbox */}
                <input 
                  type="checkbox" 
                ...
                <div className="relative h-40 w-full bg-zinc-950">
                    {project.image_url ? (
                        <Image src={project.image_url} alt={project.title} fill className="object-cover opacity-60" />
                    ) : (
      ...
      {isFormOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
             <div className="bg-[#0a0a0f] border border-zinc-800 w-full max-w-3xl rounded-2xl p-8 max-h-[90vh] overflow-y-auto relative shadow-[0_0_50px_rgba(0,0,0,0.8)]">
                <button onClick={closeForm} className="absolute top-4 right-4 text-zinc-500 hover:text-white">
                    <X size={24} />
                </button>
...
function ProjectForm({ project, onComplete }: { project?: Project | null, onComplete: () => void }) {
        ...
        <form action={handleSubmit} className="flex flex-col gap-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                 <div className="flex flex-col gap-2">
                    ...
                    <input name="title" defaultValue={project?.title} required className="p-3 bg-zinc-900 border border-zinc-700 rounded-lg text-white focus:border-cyan-500 focus:outline-none" />
                 </div>
                 <div className="flex flex-col gap-2">
                    <label className="text-sm text-zinc-400">Short Description (Card)</label>
                    <input name="short_description" defaultValue={project?.short_description || ''} required className="p-3 bg-zinc-900 border border-zinc-700 rounded-lg text-white focus:border-cyan-500 focus:outline-none" />
                 </div>
            </div>

            <div className="flex flex-col gap-2">
                <label className="text-sm text-zinc-400">Full Description</label>
                <textarea name="description" defaultValue={project?.description || ''} rows={4} className="p-3 bg-zinc-900 border border-zinc-700 rounded-lg text-white focus:border-cyan-500 focus:outline-none" />
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                 <div className="flex flex-col gap-2">
                    <label className="text-sm text-zinc-400">Repo URL</label>
                    <input name="repo_url" defaultValue={project?.repo_url || ''} type="url" className="p-3 bg-zinc-900 border border-zinc-700 rounded-lg text-white focus:border-cyan-500 focus:outline-none" />
                 </div>
                 <div className="flex flex-col gap-2">
                    <label className="text-sm text-zinc-400">Demo URL</label>
                    <input name="demo_url" defaultValue={project?.demo_url || ''} type="url" className="p-3 bg-zinc-900 border border-zinc-700 rounded-lg text-white focus:border-cyan-500 focus:outline-none" />
                 </div>
            </div>

            <div className="flex flex-col gap-2">
                <label className="text-sm text-zinc-400">Tags (comma separated)</label>
                <input name="tags" defaultValue={project?.tags?.join(', ') || ''} placeholder="Next.js, TypeScript, AI" className="p-3 bg-zinc-900 border border-zinc-700 rounded-lg text-white focus:border-cyan-500 focus:outline-none" />
            </div>
            ...
            <div className="flex flex-col gap-2">
                 ...
                 {project?.image_url && (
                    <p className="text-xs text-zinc-500 mt-1">Current image: {project.image_url.split('/').pop()}</p>
                 )}
            </div>
