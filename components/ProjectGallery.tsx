          ...
          <div className="absolute bottom-0 left-0 p-6 z-10 w-full">
            ...
            <div className="flex gap-2 mt-3 flex-wrap">
              {project.tags?.slice(0, 3).map(tag => (
                <span key={tag} className="text-xs bg-white/5 border border-white/10 text-white px-2 py-1 rounded-full backdrop-blur-md">
                  {tag}
                </span>
              ))}
            </div>
                ...
                >
                  ...
                  <div className="p-8 overflow-y-auto">
                      ...
                      <div>
                        ...
                        <div className="flex gap-2 mt-3 flex-wrap">
                           {project.tags?.map(tag => (
                             <span key={tag} className="text-xs border border-cyan-900 text-cyan-300 bg-cyan-950/30 px-2 py-1 rounded-full">
                               {tag}
                             </span>
                           ))}
                        </div>
                    ...
                    <div className="flex gap-4 flex-col sm:flex-row">
                      {project.demo_url && (
                          <a href={project.demo_url} target="_blank" rel="noopener noreferrer" className="flex-1 bg-cyan-600 hover:bg-cyan-500 text-white font-bold py-3 px-4 rounded-xl flex items-center justify-center gap-2 transition-all shadow-[0_0_20px_rgba(0,240,255,0.3)] hover:shadow-[0_0_30px_rgba(0,240,255,0.5)]">
                            <ExternalLink size={18} /> Play / Demo
                          </a>
                      )}
                      
                      {project.repo_url && (
                        <a href={project.repo_url} target="_blank" rel="noopener noreferrer" className="flex-1 bg-zinc-800 hover:bg-zinc-700 text-white font-medium py-3 px-4 rounded-xl flex items-center justify-center gap-2 transition-colors border border-zinc-700">
